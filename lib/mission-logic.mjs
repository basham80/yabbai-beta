...
/**
 * NVIDIA-hosted Kimi K2 (free tier, no card required).
 * Get a key at https://build.nvidia.com/settings/api-keys (starts with `nvapi-`).
 * Defaults to non-streaming JSON for simplicity. Override model/base via env.
 */
async function callKimiNvidia(env, { system, prompt }) {
  ...
  const model = env.NVIDIA_MODEL || 'moonshotai/kimi-k2.6';
  ...
  const text = data.choices?.[0]?.message?.content || '';
  return { text, provider: 'kimi-nvidia', model };
}
...
export async function dispatchMission(env, body) {
  ...
  // Auto: try Anthropic → NVIDIA Kimi → Moonshot
  if (hasAnthropic) {
    try {
        ...
        try {
          ...
          if (hasMoonshot) {
            ...
            return {
              ...out,
              route: 'moonshot',
              fallbackNote: `Anthropic + NVIDIA Kimi failed: ${e2.message}`,
            };
          }
          throw e2;
        }
